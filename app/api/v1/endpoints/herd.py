...
async def list_herds(
    ...
    try:
        ...
        logger.info(
            f"User {current_user.get('sub')} listed herds (skip={skip}, limit={limit})"
        )
        return result
        ...
        raise HTTPException(
            ...
            detail="Failed to retrieve herds",
        )
...
async def get_herd(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to retrieve herd",
        )
    except Exception as e:
...
async def search_herds_by_name(
    ...
    try:
        ...
        logger.info(
            f"User {current_user.get('sub')} searched herds by name '{name}', found {len(herds)} results"
        )
        return herds
    except ValidationError as e:
...
async def search_herds_by_location(
    ...
    try:
        ...
        logger.info(
            f"User {current_user.get('sub')} searched herds by location '{location}', found {len(herds)} results"
        )
        return herds
    except ValidationError as e:
...
async def create_herd(
    ...
    try:
        ...
        logger.info(
            f"User {current_user.get('sub')} created herd {herd.id}: {herd.name}"
        )
        return herd
    except ValidationError as e:
