...
class HerdRepository(BaseRepository):
    ...
    def create(self, db: Connection, herd_data: HerdCreate) -> Herd:
        ...
        logger.info(f"Created herd {herd_id}: {created_herd.name} at {created_herd.location}")
        return created_herd
    
    def update(self, db: Connection, herd_id: int, herd_data: HerdUpdate) -> Optional[Herd]:
